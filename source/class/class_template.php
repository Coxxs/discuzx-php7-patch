...
class template {
	...
	function parse_template($tplfile, $templateid, $tpldir, $file, $cachefile) {
		...
		$template = preg_replace_callback("/\{lang\s+(.+?)\}/is", 
			function ($matches) {
		...
		$template = preg_replace_callback("/\{hook\/(\w+?)(\s+(.+?))?\}/i", 
			function ($matches) {
				return $this->hooktags(isset($matches[1]) ? $matches[1] : '', isset($matches[3]) ? $matches[3] : '');
			}, $template);
