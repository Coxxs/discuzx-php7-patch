...
function mobile_parsetable($width, $bgcolor, $message) {
	if(strpos($message, '[/tr]') === FALSE && strpos($message, '[/td]') === FALSE) {
		...
		$message = preg_replace_callback("/\[tr(?:=([\(\)\s%,#\w]+))?\]\s*\[td(?:=(\d{1,4}%?))?\]/i", 
			function ($matches) { return mobile_parsetrtd($matches[1], '0', '0', $matches[2]); }, $message);
		$message = preg_replace_callback("/\[\/td\]\s*\[td(?:=(\d{1,4}%?))?\]/i", 
			function ($matches) { return mobile_parsetrtd('td', '0', '0', $matches[1]); }, $message);
		$message = preg_replace_callback("/\[tr(?:=([\(\)\s%,#\w]+))?\]\s*\[td(?:=(\d{1,2}),(\d{1,2})(?:,(\d{1,4}%?))?)?\]/i", 
			function ($matches) { return mobile_parsetrtd($matches[1], $matches[2], $matches[3], $matches[4]); }, $message);
		$message = preg_replace_callback("/\[\/td\]\s*\[td(?:=(\d{1,2}),(\d{1,2})(?:,(\d{1,4}%?))?)?\]/i", 
			function ($matches) { return mobile_parsetrtd('td', $matches[1], $matches[2], $matches[3]); }, $message);
