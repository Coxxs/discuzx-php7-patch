		...
		if(!$imgcontent) {
			...
			if($post['first']) {
				...
				if(strpos($post['message'], '[/begin]') !== FALSE) {
					$post['message'] = preg_replace_callback("/\[begin(=\s*([^\[\<\r\n]*?)\s*,(\d*),(\d*),(\d*),(\d*))?\]\s*([^\[\<\r\n]+?)\s*\[\/begin\]/is", function ($matches) use ($_G, $post) {
						return $_G['cache']['usergroups'][$post['groupid']]['allowbegincode'] ? parsebegin($matches[2], $matches[7], $matches[3], $matches[4], $matches[5], $matches[6]); : '';
					}, $post['message']);
				}
			}
		}
