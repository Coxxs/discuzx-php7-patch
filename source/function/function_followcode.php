...
function followcode($message, $tid = 0, $pid = 0, $length = 0, $allowimg = true) {
	...
	if(strpos($msglower, '[/hide]') !== FALSE) {
		preg_replace_callback("/\[hide.*?\]\s*(.*?)\s*\[\/hide\]/is", function ($matches) {
	...
	if(strpos($msglower, '[/img]') !== FALSE) {
		$message = preg_replace_callback("/\[img\]\s*([^\[\<\r\n]+?)\s*\[\/img\]/is", function ($matches) use ($extra, $allowimg) {
			return $allowimg ? fparseimg($matches[1], $extra) : '';
		}
		, $message);
		$message = preg_replace_callback("/\[img=(\d{1,4})[x|\,](\d{1,4})\]\s*([^\[\<\r\n]+?)\s*\[\/img\]/is", function ($matches) use ($extra, $allowimg) {
			return $allowimg ? fparseimg($matches[3], $extra) : '';
		}
		, $message);
	}
...
function fparsetable($width, $bgcolor, $message) {
	...
	if(strpos($message, '[/tr]') === FALSE && strpos($message, '[/td]') === FALSE) {
		...
		$message = preg_replace_callback("/\[tr(?:=([\(\)\s%,#\w]+))?\]\s*\[td(?:=(\d{1,4}%?))?\]/i", 
			function ($matches) { return parsetrtd($matches[1], '0', '0', $matches[2]); }, $message);
		$message = preg_replace_callback("/\[\/td\]\s*\[td(?:=(\d{1,4}%?))?\]/i", 
			function ($matches) { return parsetrtd('td', '0', '0', $matches[1]); }, $message);
		$message = preg_replace_callback("/\[tr(?:=([\(\)\s%,#\w]+))?\]\s*\[td(?:=(\d{1,2}),(\d{1,2})(?:,(\d{1,4}%?))?)?\]/i", 
			function ($matches) { return parsetrtd($matches[1], $matches[2], $matches[3], $matches[4]); }, $message);
		$message = preg_replace_callback("/\[\/td\]\s*\[td(?:=(\d{1,2}),(\d{1,2})(?:,(\d{1,4}%?))?)?\]/i", 
			function ($matches) { return parsetrtd('td', $matches[1], $matches[2], $matches[3]); }, $message);
		$html = '<table cellspacing="0" class="t_table" '.
			($width == '' ? NULL : 'style="width:'.$width.'"').
			($bgcolor ? ' bgcolor="'.$bgcolor.'">' : '>').
			str_replace('\\"', '"', preg_replace("/\[\/td\]\s*\[\/tr\]\s*/i", '</td></tr>', $message)).'</table>';
	}
	return fcodedisp($html);
